...
VSS_CTX_RAG_DIR = os.path.join(ROOT_DIR, "src", "vss_ctx_rag")
...
# construct an VSS_CTX_RAG package with an __init__.py file
...
shutil.copytree(VSS_CTX_RAG_DIR, os.path.join(API_TREE, "vss_ctx_rag"))
with open(os.path.join(API_TREE, "vss_ctx_rag", "__init__.py"), "w") as f:
    f.write("")
...
def skip_pydantic_special_attrs(
    app: object,
    what: str,
    name: str,
    obj: "PythonObject",
    ...
    if not skip:
        ...
        if (
            not skip
            and ("pydantic.BaseModel" in bases or "EndpointBase" in bases)
