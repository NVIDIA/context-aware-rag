...
def trigger_pipeline(
    ...
    payload_pairs: list[tuple[str, str]] = [
        ...
        ("variables[][key]", "CARAG_TARGET_BRANCH"),
...
def fetch_pr_base_ref(repo: str, pr_number: int, token: str) -> str:
    ...
    headers = {
        ...
        "User-Agent": "carag-trigger-downstream-pipeline",
    }
    if token:
...
def resolve_branches() -> tuple[str, str]:
    ...
    if not base_ref:
        ...
        print(
            ...
            "falling back to GITHUB_REF_NAME for CARAG_TARGET_BRANCH"
        )
        return ref_name, ref_name
...
def main() -> int:
    try:
        ...
        variable_name = os.environ.get("DOWNSTREAM_SUBMODULE_HASH_VARIABLE", "CARAG_SUBMODULE_HASH")
